...
public class ExpansionSample {
	
	public static String graphFileName = "ExpansionSampleGraph.txt";
	public static String queryOutputDirName = "/ExpansionSample";
	public static String outputAnsDirName = "./ExpansionSampleOutputAns";
	...
	public static void main(String[] args)
	{
		if (args.length != 4)
		{
			...
			System.out.println("Usage: java -cp SNA_HW3.jar ExpansionSample <graph outputfile name> <query outputDirectory name> <query times> <output ans dir>");
			System.out.println("Using default args: java -cp SNA_HW3.jar ExpansionSample "+graphFileName+" "+queryOutputDirName+" "+queryNum+" "+outputAnsDirName);
		}
		else
		{
	...
	public static Edge biggestExpansionNeighbor(GraphManipulation gm, Node currentNode, ArrayList<Edge> neighbors)
	{
		...
		for (Node node:gm.getVertices())
			if (((ExpansionNode)node).neighbors.contains(biggest.node2));
		...
		for (Edge n:neighbors)
		{
			...
			else
			{
				...
				for (Node node:gm.getVertices())
					if (((ExpansionNode)node).neighbors.contains(n.node2))
